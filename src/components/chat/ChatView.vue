<template>
  <div class="flex flex-col h-full overflow-hidden bg-[var(--bg-chat)] relative">
    ...
    >
      <div class="max-w-4xl mx-auto w-full min-h-full pb-12 flex flex-col">
        ...
        <div v-if="chatStore.activeSystemPrompt" class="px-4 pt-6 mb-2">
          <div class="bg-[var(--bg-surface)] border border-[var(--border-muted)] rounded-xl shadow-sm overflow-hidden">
            ...
            >
              <div class="flex-shrink-0 p-1.5 bg-[var(--accent-muted)] rounded-lg">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#4a80d0" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/>
                </svg>
              </div>
              <span class="text-[11px] text-[var(--text-muted)] font-mono uppercase tracking-wider flex-1">System Instructions</span>
              <!-- Collapsed preview -->
              <span v-if="!isSystemPromptExpanded" class="text-[12px] text-[var(--text-muted)] italic truncate max-w-xs opacity-60">
            ...
            <div v-if="isSystemPromptExpanded" class="px-4 pb-4">
              <div class="text-[13px] text-[var(--text-h)] leading-relaxed italic opacity-80 whitespace-pre-wrap">
          ...
          <template #default="{ item, index, active }">
            ...
            >
              <div class="flex items-center gap-3 py-3 px-6">
                ...
                <span class="text-[10px] text-amber-400/60 font-medium whitespace-nowrap select-none">
        ...
        <div v-else class="flex-1 flex flex-col items-center justify-center">
          <div class="w-32 h-32 flex items-center justify-center">
            <img src="../../assets/llama-main.png" alt="Ollama" class="w-full h-full object-contain themed-logo opacity-80" />
          </div>
        </div>
      </div>
    </div>
    ...
    <transition name="slide-up">
      ...
      >
        <div class="mx-4 bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl px-4 py-3 shadow-[var(--shadow)]">
          <div class="flex items-center gap-2.5 mb-2">
            <svg class="w-4 h-4 text-[var(--text-muted)] flex-shrink-0 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M21 12a9 9 0 1 1-6.219-8.56" stroke-linecap="round"/>
            </svg>
            <span class="text-[13px] text-[var(--text-h)] font-medium truncate">Downloading {{ activePullProgress.model }}</span>
            <span class="text-[12px] text-[var(--text-muted)] ml-auto flex-shrink-0">{{ activePullProgress.status }}</span>
          </div>
          ...
          <div v-if="activePullProgress.percent > 0" class="text-[11px] text-[var(--text-muted)] mt-1 text-right">
...
function isBoundaryItem(item: unknown): item is BoundaryItem {
  return !!item && typeof item === 'object' && 'type' in item && (item as Record<string, unknown>).type === 'context-boundary'
}
...
const nonSystemMessages = computed(() => messages.value.filter(m => m.role !== 'system'))
...
const activeModelName = computed(() => chatStore.activeConversation?.model ?? '')

const maxContextForBoundary = computed(() => {
  const globalSetting = settingsStore.chatOptions.num_ctx
  const modelNative = modelStore.getCapabilities(activeModelName.value)?.context_length
...
const itemsForScroller = computed<(ScrollerItem | BoundaryItem)[]>(() => {
  ...
  if (boundaryIdx > 0 && boundaryIdx < items.length) {
    items.splice(boundaryIdx, 0, { id: 'context-boundary', type: 'context-boundary' })
  }
...
async function onSend(text: string, images?: Uint8Array[], webSearchEnabled?: boolean, thinkMode?: string) {
