...
use ollama_desktop_lib::commands::models::{list_models, pull_model, delete_model};
...
async fn test_list_models_integration() {
    ...
    db::hosts::create(&direct_conn, NewHost {
        name: "Test Host".into(),
        url: url.clone(),
...
"#)
    ...
    app_handle.listen("model:pull-progress", move |event| {
        events_clone.lock().unwrap().push(event.payload().to_string());
    });
